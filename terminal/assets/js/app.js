const SUPABASE_URL = 'https://bwajmlxxmxamwneyebax.supabase.co';
    ...
    const BUILD_VERSION = 'v5.7.5 [MESAJ YETKİSİ]';
    ...
    async function fetchRaps() { if (window.F_fetchRaps) return ALL_RAPS; window.F_fetchRaps = true;  const { data } = await _supabase.from('raporlar').select('*').eq('is_deleted', false).order('created_at', { ascending: false }); ALL_RAPS = (data || []).filter(r => r.scd !== 'GUNLUK').map(dbToRap); return ALL_RAPS; }
    ...
    async function fetchGRaps() { if (window.F_fetchGRaps) return ALL_GRAPS; window.F_fetchGRaps = true; const { data } = await _supabase.from('raporlar').select('*').eq('is_deleted', false).eq('scd', 'GUNLUK').order('created_at', { ascending: false }); ALL_GRAPS = (data || []).map(dbToGRap); return ALL_GRAPS; }
    async function insertGRap(rap) { const { data, error } = await _supabase.from('raporlar').insert([gRapToDb(rap)]).select().single(); if (error) return null; const mapped = dbToGRap(data); ALL_GRAPS.unshift(mapped); return mapped; }
    async function updateGRap(id, fields) { const { error } = await _supabase.from('raporlar').update(fields).eq('id', id); if (!error) { const i = ALL_GRAPS.findIndex(x => x.id === id); if (i >= 0) Object.assign(ALL_GRAPS[i], fields); } }
    async function deleteGRap(id) { const { error } = await _supabase.from('raporlar').update({ is_deleted: true }).eq('id', id); if (!error) ALL_GRAPS = ALL_GRAPS.filter(x => x.id !== id); }
    ...
    function dbToGRap(r) { let p = {}; try { p = JSON.parse(r.amac || '{}'); } catch(e){} return { id: r.id, username: r.username, displayName: r.display_name, dNo: r.d_no, robloxAdi: p.robloxAdi||'', rutbe: r.rutbe, rutbeKat: p.rutbeKat||'', deneyler: p.deneyler||'', sure: p.sure||'', ekler: p.ekler||'', mesaiImgs: p.mesaiImgs||[], deneyImgs: p.deneyImgs||[], onayDurum: r.onay_durum || 'bekliyor', onayYapan: r.onay_yapan, onayTarih: r.onay_tarih, tarih: r.created_at ? new Date(r.created_at).toLocaleString('tr-TR') : '—', rawDate: r.created_at }; }
    function gRapToDb(r) { const p = JSON.stringify({ robloxAdi: r.robloxAdi, rutbeKat: r.rutbeKat, deneyler: r.deneyler, sure: r.sure, ekler: r.ekler, mesaiImgs: r.mesaiImgs, deneyImgs: r.deneyImgs }); return { user_id: CU.userId, username: r.username, display_name: r.displayName, d_no: r.dNo, rutbe: r.rutbe, scd: 'GUNLUK', scp: 'RAPOR', amac: p, sonuc: '', onay_durum: 'bekliyor' }; }
    ...
    async function fetchRutbeler() { 
      try {
        ...
        if (data && data.length > 0) {
          ...
          RUTBE_LIST = JSON.parse(JSON.stringify(DEFAULT_RUTBELER));
          RUTBE_LIST.forEach((r, idx) => r.id = 'fb_' + idx);
          if (!error) {
            _supabase.from('rutbeler').insert(DEFAULT_RUTBELER).then(()=>{});
          } else {
            console.error('Supabase Rütbeler Tablosu Hatası:', error);
            alert('UYARI: Supabase "rutbeler" tablosu bulunamadı veya erişilemedi. Lütfen SQL scriptini Supabase üzerinde çalıştırdığınızdan emin olun. Rütbeler geçici olarak hafızadan yüklendi.');
          }
        }
      } catch (err) {
        RUTBE_LIST = JSON.parse(JSON.stringify(DEFAULT_RUTBELER));
...
function getGelenMesajlar() { if (!CU) return []; return ALL_MESAJLAR.filter(m => m.alici_un === CU.username || m.alici_tip === 'toplu_herkes' || (m.alici_tip === 'toplu_yetkisiz' && CU.role === 'yetkisiz') || (m.alici_tip === 'toplu_yetkili' && CU.role === 'yetkili')); }
    ...
    (async function initSession() {
      ...
      await Promise.all([fetchMesajlar(), fetchRutbeler()]); 
      buildSidebar();
    ...
    function buildSidebar() {
      ...
      const iletisimHtml = `<div class="sb-label" style="margin-top:8px">İLETİŞİM</div>
           ...
           ${CU.role !== 'yetkisiz' ? `<button class="sb-item" id="nav-mesaj-gonder" onclick="showPage('mesaj-gonder')">MESAJ GÖNDER</button>` : ''}`;
      ...
      if (CU.role === 'yetkisiz') {
          ...
          content = `<div class="sb-label">YETKİLİ ERİŞİMİ</div>
           ...
           <button class="sb-item" id="nav-rutbe-yonetimi" onclick="showPage('rutbe-yonetimi')">RÜTBE YÖNETİMİ</button>
    ...
    async function showPage(id, skipHistory = false) {
      ...
      try {
        ...
        else if (id === 'rutbe-yonetimi') { ca.innerHTML = bRutbeYonetimi(); }
    ...
    function bRutbeYonetimi() { let h = `<div><div class="ph"><h2>RÜTBE YÖNETİMİ</h2></div><div style="margin-bottom:20px;"><button class="sub-btn" style="width:auto;padding:5px 15px;" onclick="openRutbeModal()">[ YENİ RÜTBE OLUŞTUR ]</button></div><div class="rlist" id="rutbe-list">`; RUTBE_LIST.filter(r => !r.disabled).forEach(r => { h += `<div class="rc" style="display:flex;justify-content:space-between;align-items:center;border-left-color:${RC[r.kat] || 'var(--green)'}"><div><div class="rc-id" style="${r.disabled ? 'text-decoration:line-through;color:var(--gray)' : ''}">${r.label}</div><div class="rc-meta">Kategori: ${r.kat.toUpperCase()} | Ayrıştırıcı/Devre Dışı: ${r.disabled ? 'EVET' : 'HAYIR'}</div></div><div style="display:flex;gap:6px"><button class="exp-btn" onclick="openRutbeModal('${r.id}')">[ DÜZENLE ]</button><button class="sil-btn" onclick="silRutbe('${r.id}')">[ SİL ]</button></div></div>`; }); return h + `</div></div>`; }
    function openRutbeModal(id = null) { if (String(id).startsWith('fb_')) { alert("UYARI: Bu rütbe henüz veritabanında değil, geçici hafızada! Düzenleyebilmek için lütfen Supabase SQL kurulumunu tamamlayın."); return; } let r = id ? RUTBE_LIST.find(x => x.id == id) : { kat: 'lr', label: '', disabled: false }; document.getElementById('mt').textContent = id ? `RÜTBE DÜZENLE // ${r.label}` : `YENİ RÜTBE OLUŞTUR`; document.getElementById('mo-sil-btn').style.display = 'none'; let b = `<div class="rf" style="border:none;padding:0;max-width:100%"><div class="fs"><div class="fg"><label>RÜTBE ADI (LABEL)</label><input type="text" class="tin" id="er-label" value="${r.label}"/></div><div class="fg"><label>KATEGORİ (kat)</label><select class="ts" id="er-kat"><option value="hc" ${r.kat==='hc'?'selected':''}>HC</option><option value="hr" ${r.kat==='hr'?'selected':''}>HR</option><option value="mr" ${r.kat==='mr'?'selected':''}>MR</option><option value="lr" ${r.kat==='lr'?'selected':''}>LR</option></select></div><div class="fg"><label>AYRIŞTIRICI / DEVRE DIŞI</label><select class="ts" id="er-disabled"><option value="false" ${!r.disabled?'selected':''}>HAYIR (Seçilebilir Rütbe)</option><option value="true" ${r.disabled?'selected':''}>EVET (Menü Başlığı vb.)</option></select></div></div><button class="sub-btn" id="er-btn" onclick="saveRutbe(${id ? `'${id}'` : 'null'})">[ ${id ? 'GÜNCELLE' : 'OLUŞTUR'} ]</button><div id="er-result" style="margin-top:10px;font-size:11px"></div></div>`; document.getElementById('mbody').innerHTML = b; document.getElementById('mo').classList.add('active'); }
    async function saveRutbe(id) { const btn = document.getElementById('er-btn'); const res = document.getElementById('er-result'); btn.disabled = true; res.textContent = "Kaydediliyor..."; const label = document.getElementById('er-label').value.trim(); const kat = document.getElementById('er-kat').value; const disabled = document.getElementById('er-disabled').value === 'true'; if(!label) { res.textContent = "Rütbe adı boş olamaz."; btn.disabled = false; return; } let error = null; if (id) { const r = await _supabase.from('rutbeler').update({ label, kat, disabled }).eq('id', id); error = r.error; } else { const r = await _supabase.from('rutbeler').insert([{ label, kat, disabled }]); error = r.error; } if(!error) { res.textContent = "Başarılı!"; await fetchRutbeler(); showPage('rutbe-yonetimi'); document.getElementById('mo').classList.remove('active'); } else { res.textContent = "Hata oluştu."; btn.disabled = false; } }
    async function silRutbe(id) { if (String(id).startsWith('fb_')) { alert("UYARI: Bu rütbe henüz veritabanında değil, geçici hafızada! Silebilmek için lütfen Supabase SQL kurulumunu tamamlayın."); return; } if(!confirm("Bu rütbeyi silmek istediğinize emin misiniz?")) return; const { error } = await _supabase.from('rutbeler').delete().eq('id', id); if(!error) { await fetchRutbeler(); showPage('rutbe-yonetimi'); } else { alert("Silinirken hata oluştu."); } }
