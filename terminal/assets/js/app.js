const SUPABASE_URL = 'https://bwajmlxxmxamwneyebax.supabase.co';
    ...
    const BUILD_VERSION = 'v5.2 [GLASSMORPHISM UI REDESIGN]';
...
function getGelenMesajlar() { if (!CU) return []; return ALL_MESAJLAR.filter(m => m.alici_un === CU.username || m.alici_tip === 'toplu_herkes' || (m.alici_tip === 'toplu_yetkisiz' && CU.role === 'yetkisiz') || (m.alici_tip === 'toplu_yetkili' && CU.role === 'yetkili')); }
    ...
    function bTumRaporlar() { const raps = getRaps(); let bekleyenSayi = raps.filter(r => r.onayDurum === 'bekliyor').length; let h = `<div><div class="dashboard-welcome"><h1>HOŞ GELDİNİZ, <span>${CU.displayName.split(' ')[0].toUpperCase()}</span>.</h1><p>Onayınızı bekleyen <strong>${bekleyenSayi}</strong> adet hayati rapor bulunuyor.</p><div class="sys-status">TESİS GÜVENDE • SİSTEMLER AKTİF</div><div class="bg-triangle">⚠</div></div><div class="ph" style="margin-top:50px;"><h2>SON RAPORLAR</h2></div><div class="rlist">`; if (!raps.length) h += `<div class="empty">VERİ BULUNAMADI.</div>`; else { h += getSortSelectHtml(); sortRaps(raps.slice()).forEach(r => h += rCard(r, true)); } return h + `</div></div>`; }
