const SUPABASE_URL = 'https://bwajmlxxmxamwneyebax.supabase.co';
    ...
    const BUILD_VERSION = 'v5.7.2 [RÜTBE YÖNETİMİ]';
...
function getGelenMesajlar() { if (!CU) return []; return ALL_MESAJLAR.filter(m => m.alici_un === CU.username || m.alici_tip === 'toplu_herkes' || (m.alici_tip === 'toplu_yetkisiz' && CU.role === 'yetkisiz') || (m.alici_tip === 'toplu_yetkili' && CU.role === 'yetkili')); }
    ...
    async function silRutbe(id) { if(!confirm("Bu rütbeyi silmek istediğinize emin misiniz?")) return; const { error } = await _supabase.from('rutbeler').delete().eq('id', id); if(!error) { await fetchRutbeler(); showPage('rutbe-yonetimi'); } else { alert("Silinirken hata oluştu."); } }
