...
function getGelenMesajlar() { if (!CU) return []; return ALL_MESAJLAR.filter(m => m.alici_un === CU.username || m.alici_tip === 'toplu_herkes' || (m.alici_tip === 'toplu_yetkisiz' && CU.role === 'yetkisiz') || (m.alici_tip === 'toplu_yetkili' && CU.role === 'yetkili')); }
    ...
    function bHesapOlustur() { return `<div><div class="ph"><h2>HESAP OLUŞTUR</h2><p>Sisteme yeni personel kaydı oluşturun</p></div><div class="suc" id="ho-suc">PERSONEL HESABI OLUŞTURULDU.</div><div class="form-err-box" id="ho-form-err"></div><div class="rf"><div class="fs"><div class="fs-title">Hesap Bilgileri</div><div class="frow"><div class="fg"><label>KULLANICI ADI</label><input type="text" class="tin" id="ho-username"/></div><div class="fg"><label>ŞİFRE</label><input type="password" class="tin" id="ho-password"/></div></div><div class="fg"><label>AD SOYAD</label><input type="text" class="tin" id="ho-isim"/></div></div><div class="fs"><div class="fs-title">Yetki & Roller</div><div class="frow"><div class="fg"><label>RÜTBE</label><select class="ts" id="ho-rutbe" onchange="upRutbeClrHo()">${rutbeOpts()}</select></div><div class="fg"><label>YETKİ TÜRÜ</label><select class="ts" id="ho-role"><option value="yetkisiz">Yetkisiz</option><option value="yetkili">Yetkili Admin</option></select></div></div></div><button class="sub-btn" onclick="gonderHesapOlustur()" id="ho-btn">KAYDI TAMAMLA</button></div></div>`; }
    ...
    function bPersonelYonetimi() { let h = `<div><div class="ph"><h2>PERSONEL YÖNETİMİ</h2><p>Personel kayıtlarını düzenle ve yönet</p></div><div class="rlist">`; ALL_PROFILES.forEach(p => { h += `<div class="rc" style="display:flex;justify-content:space-between;align-items:center;"><div><div class="rc-id">${p.username}</div><div class="rc-meta">${p.displayName} · <span style="color:var(--amber)">${p.role.toUpperCase()}</span> · ${p.rutbe}</div></div><div style="display:flex;gap:6px"><button class="exp-btn" onclick="openEditModal('${p.username}')">DÜZENLE</button>${p.username !== CU.username ? `<button class="sil-btn" onclick="openDeleteUserModal('${p.username}')">SİL</button>` : ''}</div></div>`; }); return h + `</div></div>`; }
    ...
    function bRaporYaz() { if (isS1()) return `<div><div class="ph"><h2>YENİ RAPOR OLUŞTUR</h2></div><div style="padding:20px;text-align:center;color:var(--red)">⚠ ERİŞİM REDDEDİLDİ</div></div>`; const nextNo = fmtNo(userRapCount(CU.username) + 1); const autoRutbe = CU.rutbe || ''; const rutbeColor = CU.rutbeKat ? (RC[CU.rutbeKat] || 'var(--green)') : 'var(--green)'; return `<div><div class="ph"><h2>YENİ RAPOR OLUŞTUR</h2><p>Lütfen sisteme girmek istediğiniz rapor türünü seçin</p></div><div style="display:flex; gap:30px;"><div class="sidebar show"><button class="sb-item active" onclick="showPage('rapor-yaz')">Deney Raporu</button><button class="sb-item" onclick="showPage('gunluk-yaz')">Günlük Rapor</button><button class="sb-item" onclick="showPage('kaza-yaz')">Kaza Raporu</button><button class="sb-item" onclick="showPage('muhafaza-yaz')">Muhafaza Raporu</button></div><div style="flex:1;"><div class="suc" id="suc">BAŞARILI.</div><div class="form-err-box" id="form-err"></div><div class="rf"><div class="fs"><div class="fs-title">Deney Raporu Formu</div><div class="frow"><div class="fg"><label>KOD</label><select class="ts" id="r-scd">${scdOpts()}</select></div><div class="fg"><label>NO</label><input type="text" class="tin" value="${nextNo}" readonly/></div></div><div class="frow"><div class="fg"><label>SCP</label><select class="ts" id="r-scp">${scpOpts()}</select></div><div class="fg"><label>SORUMLU</label><input type="text" class="tin" id="r-sor"/></div></div><div class="fg"><label>RÜTBE</label><input type="text" class="tin" id="r-rutbe" value="${autoRutbe}" readonly style="color:${rutbeColor}"/></div><div class="frow"><div class="fg"><label>BAŞLANGIÇ</label><input type="time" class="ttm" id="r-zb"/></div><div class="fg"><label>BİTİŞ</label><input type="time" class="ttm" id="r-ze"/></div></div><div class="fg"><label>SINIF-D</label><input type="number" class="tn" id="r-sd"/></div></div><div class="fs"><div class="fg"><label>AMACI</label><textarea class="ta" id="r-amac"></textarea></div><div class="fg"><label>SONUCU</label><textarea class="ta" id="r-sonuc"></textarea></div></div><div class="fs"><div class="ts-list" id="tsl"></div><button class="add-ts" onclick="addTs()">ZAMAN DAMGASI EKLE</button></div><div class="fs"><div class="fg"><label>ANOMALİLER</label><textarea class="ta" id="r-ano"></textarea></div><div class="fg"><label>EKLER</label><textarea class="ta" id="r-ek"></textarea></div></div><div class="fs"><div class="img-area" onclick="document.getElementById('img-in').click()"><input type="file" id="img-in" accept="image/*" multiple onchange="handleImgs(event)"/><label class="img-lbl">Dosya seçmek için tıklayın veya sürükleyin</label><div class="img-prev" id="imgprev"></div></div></div><button class="sub-btn" onclick="gonder()">GÖNDER</button><div style="clear:both;"></div></div></div></div></div>`; }
    ...
    function bRaporlarim() { const raps = getRaps().filter(r => r.username === CU.username); let h = `<div><div class="ph"><h2>RAPORLARIM</h2><p>Tüm raporlarınız</p></div><div class="rlist">`; if (!raps.length) h += `<div class="empty">VERİ BULUNAMADI.</div>`; else { h += getSortSelectHtml(); sortRaps(raps.slice()).forEach(r => h += rCard(r, false)); } return h + `</div></div>`; }
    function bTumRaporlar() { const raps = getRaps(); let bekleyenSayi = raps.filter(r => r.onayDurum === 'bekliyor').length; let h = `<div><div class="dashboard-welcome"><h1>HOŞ GELDİNİZ, <span>${CU.displayName.split(' ')[0].toUpperCase()}</span>.</h1><p>Onayınızı bekleyen <strong>${bekleyenSayi}</strong> adet hayati rapor bulunuyor.</p><div class="sys-status">TESİS GÜVENDE • SİSTEMLER AKTİF</div><div class="bg-triangle">⚠</div></div><div class="node-map-widget"><div class="node-map-content"><div class="mock-node large"></div><div class="mock-node small n1"></div><div class="mock-node small n2"></div><div class="mock-node small n3"></div><div class="mock-node-line l1"></div><div class="mock-node-line l2"></div><div class="mock-node-line l3"></div></div></div><div class="ph" style="margin-top:50px;"><h2>SON RAPORLAR</h2></div><div class="rlist">`; if (!raps.length) h += `<div class="empty">VERİ BULUNAMADI.</div>`; else { h += getSortSelectHtml(); sortRaps(raps.slice()).forEach(r => h += rCard(r, true)); } return h + `</div></div>`; }
    function rCard(r, su) { return `<div class="rc"><div class="rc-hdr"><div><div class="rc-id">${r.scd || '—'} // DENEY-${r.dNo || '000'}</div><div class="rc-meta">${su ? `<span style="color:var(--text-main)">${r.displayName}</span> · ` : ''}${r.tarih}</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px"><span class="rc-scp">${r.scp ? r.scp.split('–')[0].trim() : '—'}</span>${onayDurumBadge(r.onayDurum)}</div></div><div class="rc-body">${r.amac ? r.amac.substring(0, 120) + '…' : ''}</div><div style="display:flex;gap:6px;margin-top:8px"><button class="exp-btn" onclick="openMo(${r.id})">TAM RAPORU GÖRÜNTÜLE</button>${(CU.role === 'yetkili') ? `<button class="sil-btn" onclick="silRapor(${r.id})">SİL</button>` : ''}</div></div>`; }
...
function bAyarlar() {
    ...
    return `<div class="ph"><h2>SİSTEM AYARLARI</h2><p>Terminal Tercihleri ve Özelleştirmeler</p></div>
    <div class="settings-grid" style="margin-top:16px;">
        <div class="rf" style="padding:20px;">
            <div class="fs-title">Hesap Bilgileri</div>
            <div class="fs">
                <div class="fg"><label>GÖRÜNEN AD</label><input type="text" class="tin" value="${CU.displayName}" readonly style="opacity:0.7"/></div>
                <div class="fg"><label>PROFİL FOTOĞRAFI (Yükle veya URL Gir)</label><div style="display:flex;gap:10px;align-items:center;"><div class="user-avatar"><svg viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg></div><input type="text" class="tin" placeholder="https://... veya resim seçin"/></div></div>
                <div class="fg"><label>YETKİ SEVİYESİ (Değiştirilemez)</label><input type="text" class="tin" value="${CU.rutbe} - ${CU.role.toUpperCase()}" readonly style="opacity:0.7"/></div>
                <button class="sub-btn" style="width:100%; float:none; margin-top:10px;">KAYDET</button>
            </div>
        </div>
        
        <div style="display:flex; flex-direction:column; gap:20px;">
            <div class="rf" style="padding:20px;">
                <div class="fs-title">Bağlı Hesaplar</div>
                <div class="fs" style="margin-bottom:0;">
                    <div class="fg"><label style="color:#5865F2;">DISCORD ID</label><input type="text" class="tin" placeholder="KullanıcıAdı#1234"/></div>
                    <div class="fg"><label style="color:var(--text-main);">ROBLOX ID</label><input type="text" class="tin" value="${CU.robloxId || ''}" placeholder="Roblox Kullanıcı Adı"/></div>
                    <button class="sub-btn" style="width:100%; float:none; margin-top:0;">BAĞLANTILARI KAYDET</button>
                </div>
            </div>
            
            <div class="rf" style="padding:20px;">
                <div class="fs-title">Ses ve Medya</div>
                <div class="fs" style="margin-bottom:0;">
                    <div class="switch-row">
                        <div class="switch-label">
                            <svg viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>
                            <div class="switch-text"><strong>Arayüz Ses Efektleri</strong><span>Tıklama ve menü geçiş sesleri</span></div>
                        </div>
                        <input type="checkbox" id="set-sfx" onchange="saveSettings()" ${sfx ? 'checked' : ''} style="display:none;" />
                        <button class="switch-btn" onclick="document.getElementById('set-sfx').click(); this.textContent = document.getElementById('set-sfx').checked ? 'AÇIK' : 'KAPALI'; this.style.color = document.getElementById('set-sfx').checked ? '#4caf50' : 'var(--text-muted)'; this.style.borderColor = document.getElementById('set-sfx').checked ? 'rgba(76, 175, 80, 0.5)' : 'rgba(255, 255, 255, 0.2)';">${sfx ? 'AÇIK' : 'KAPALI'}</button>
                    </div>
                </div>
            </div>
        </div>
...
async function playBootSequence() {
    const lines = [
        ...
        "OK",
        "ESTABLISHING SECURE CONNECTION TO SITE-62...",
        "OK",
    ...
    const settings = JSON.parse(localStorage.getItem('t62_settings') || '{}');
    if (settings.fastBoot) return;
