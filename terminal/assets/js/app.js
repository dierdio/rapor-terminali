const SUPABASE_URL = 'https://bwajmlxxmxamwneyebax.supabase.co';
    ...
    const BUILD_VERSION = 'v5.4 [PORTFOLIO/LAZY YÜKLEME]';
    ...
    async function fetchGRaps() { if (window.F_fetchGRaps) return ALL_GRAPS; window.F_fetchGRaps = true; let res = await _supabase.from('gunluk_raporlar').select('*').eq('is_deleted', false).order('created_at', { ascending: false }); if(res.error) { res = await _supabase.from('gunluk_raporlar').select('*').order('created_at', { ascending: false }); } ALL_GRAPS = (res.data || []).map(dbToGRap); return ALL_GRAPS; }
